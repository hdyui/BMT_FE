...
function Metadata({ job }: { job: CareerJob }) {
  ...
  return (
    <ul className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-base text-neutral-600 sm:text-lg lg:text-xl">
...
function JobDivider({ delay }: { delay: number }) {
  ...
  return (
    <div
      ref={dividerRef}
      className="relative mt-5 aspect-[5010/123] w-full overflow-hidden"
      aria-hidden="true"
    >
      <Image
      ...
      <article className="group py-5 first:pt-0">
        <div className="grid gap-5 md:grid-cols-[34%_1fr] lg:gap-7">
          ...
          <div className="flex min-w-0 flex-col pt-0.5">
            ...
            <Metadata job={job} />
            <p className="mt-3 max-w-[780px] text-base leading-[1.55] text-neutral-700 sm:text-lg lg:text-xl">
              {job.summary}
            </p>

            <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
              <button
                className="inline-flex items-center gap-0.5 border-b border-charcoal text-[17px] leading-5 text-charcoal transition-[border-color,color] duration-300 hover:border-brand hover:text-brand"
                type="button"
                onClick={onToggle}
                aria-expanded={open}
              ...
              >
                Xem chi tiết
                <ChevronDown
                  className={`size-3.5 transition-transform duration-300 ${open ? "rotate-0" : "-rotate-90"}`}
                  aria-hidden="true"
                />
              </button>
              <a
            ...
            >
              <div className="overflow-hidden">
                <div
                  className={`pt-4 text-base leading-[1.55] text-neutral-700 transition-[opacity,transform] duration-500 ease-out sm:text-lg lg:text-xl ${open ? "translate-y-0 opacity-100 delay-100" : "translate-y-3 opacity-0 delay-0"}`}
                >
  ...
  return (
    ...
    >
      <div className="mx-auto w-[min(1180px,calc(100%-2.25rem))]">
        <Reveal>
          <div className="mx-auto flex max-w-[1100px] flex-col items-center text-center">
            <h2
              ...
              className="text-[clamp(2.25rem,3.5vw,3.3rem)] leading-[.96] font-extrabold uppercase tracking-[-.045em] text-charcoal lg:whitespace-nowrap"
            >
        ...
        <div
          className={`mt-12 will-change-[opacity,transform] transition-[opacity,transform] duration-[360ms] ease-out ${leaving ? "translate-y-2 opacity-0" : "translate-y-0 opacity-100"}`}
          aria-live="polite"
          aria-busy={leaving}
        >
        ...
        <nav
          className={styles.careerPagination}
          aria-label="Phân trang tuyển dụng"
        >
          <button
            className={styles.careerPageButton}
            type="button"
            onClick={() => changePage(page - 1)}
            disabled={page === 0 || leaving}
          >
            <Image
              className={styles.careerPageButtonIcon}
              src="/images/careers/page-previous.jpg"
              alt=""
              width={104}
              height={104}
              aria-hidden="true"
          ...
          <span className={styles.careerPageIndicator}>
            Page {page + 1}/{pageCount}
          </span>
          <button
            className={`${styles.careerPageButton} ${styles.careerPageButtonNext}`}
            type="button"
            onClick={() => changePage(page + 1)}
            disabled={page === pageCount - 1 || leaving}
          >
            <span className="hidden sm:inline">NEXT PAGE</span>
            <span className="sm:hidden">SAU</span>
            <Image
              className={styles.careerPageButtonIcon}
              src="/images/careers/page-next.jpg"
              alt=""
              width={104}
              height={104}
              aria-hidden="true"
